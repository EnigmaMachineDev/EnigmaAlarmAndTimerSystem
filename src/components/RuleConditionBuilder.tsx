...
import { RuleCondition, ConditionType, DayKey } from '../types';
import { generateId } from '../utils/uuid';
import { useAppStore } from '../store/useAppStore';
import { DAY_KEYS, DAY_LABELS } from '../constants/defaults';
...
export function RuleConditionBuilder({ conditions, onChange }: Props) {
  ...
  const [editing, setEditing] = useState<{ type: ConditionType; value: string | string[]; index: number | null } | null>(null);
  const [typePickerVisible, setTypePickerVisible] = useState(false);
  const [pendingType, setPendingType] = useState<ConditionType>('TOMORROW_PRESET_IS');

  function openNew() {
    setEditing({ type: 'TOMORROW_PRESET_IS', value: presets[0]?.id ?? '', index: null });
  }

  function openEdit(condition: RuleCondition, index: number) {
    setEditing({ type: condition.type, value: condition.value, index });
  }

  function save() {
    if (!editing) return;
    const newCondition: RuleCondition = { type: editing.type, value: editing.value };
  ...
  function describeCondition(c: RuleCondition): string {
    ...
    if (def.inputType === 'preset') {
      ...
      return `${def.label}: ${p?.name ?? 'Unknown'}`;
    }
    if (def.inputType === 'days') {
      const days = Array.isArray(c.value) ? c.value : [c.value];
      return `${def.label}: ${days.join(', ')}`;
    }
    return `${def.label} ${c.value}`;
  }
  ...
  return (
    <View>
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>Conditions (AND)</Text>
          ...
          <View style={styles.subModalSheet}>
            {CONDITION_TYPES.map((t) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (editing) {
                    const defaultVal = t.inputType === 'preset' ? (presets[0]?.id ?? '') : t.inputType === 'days' ? [] : '';
...
const styles = StyleSheet.create({
  ...
  conditionMain: { flex: 1 },
  conditionText: { fontSize: 14, color: Colors.text },
  deleteBtn: { padding: 4 },
